...
export default function PointCard({ point }: PointCardProps) {
  return (
    <Link
      to={`/point/${point.id}`}
      className="flex items-center gap-3 px-3 py-2.5 bg-gray-200 rounded-lg border border-gray-300 shadow-sm hover:shadow-md transition-all"
    >
      {/* Point image or placeholder */}
      <div className="w-11 h-11 rounded-md bg-gray-50 border border-gray-200 flex items-center justify-center flex-shrink-0 overflow-hidden">
        {point.imageId ? (
          <img
