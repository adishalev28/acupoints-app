  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg pb-24">
      ...
      <div className="px-4 py-4 space-y-3">
        ...
        {/* 4. התוויות מאסטר דונג — הכרטיס הראשון באפליקציה של שון גודמן, כלומר
            מה שדונג עצמו כתב, להבדיל מתוספות של מטפלים מאוחרים. מוצג בנפרד
            כי ההבחנה הזו היא כל מטרת החילוץ. */}
        {point.dongIndications && point.dongIndications.length > 0 && (
          <Section
            icon={
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
            ...
            title="התוויות מאסטר דונג"
          >
            <div className="flex flex-wrap gap-1.5">
              {point.dongIndications.map((item, i) => (
                <span
                  key={i}
                  className="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-semibold border bg-amber-50 text-amber-900 border-amber-300 dark:bg-amber-950/40 dark:text-amber-200 dark:border-amber-800"
                >
                  {item}
                </span>
              ))}
            </div>
            <p className="mt-2.5 text-[11px] leading-relaxed text-gray-500 dark:text-dark-muted">
              ההתוויות שמאסטר דונג עצמו כתב. הרשימה המלאה למטה כוללת גם תוספות של מטפלים מאוחרים.
            </p>
          </Section>
        )}
