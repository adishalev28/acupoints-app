...
16. **אם האפליקציה נסגרת באמצע ריצה, הסקריפט ממשיך ללחוץ על קואורדינטות
    עיוורות** ומייצר צילומים ריקים — בלי לדווח על שגיאה. הריצה מסתיימת
    ב-`DONE` ובקוד יציאה 0 כרגיל, ולכן **אין להסתמך על קוד היציאה**.

    בדיקת שלמות זולה: בלכידה תקינה הכותרת נקראת גם בצילום ה**אחרון** של
    הנקודה. אם היא ריקה שם — הלכידה נחתכה ויש למחוק אותה:

    ```bash
    LAST=$(ls "$D/shots" | grep "^q02_" | grep -v fields | sort | tail -1)
    powershell -NoProfile -File $SP/ocr.ps1 -Path "$D/shots/$LAST" -Crop "0,195,1920,120"
    ```

    לעצור מיד כשמתגלה שהאפליקציה נסגרה — לחיצות עיוורות עלולות לפגוע
    בנתוני המשתמש (הכוכב, "Add a Note")
